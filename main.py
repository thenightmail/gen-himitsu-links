...
for i, page in enumerate(pages):
    if i % 10 == 0 and i + 10 < len(pages):
        queue = ','.join(page.comment.translate(translate_table)[:11]) # the queue
        #print(queue)
        save = pages[i+10].comment # the piece that we save into the next PC
        save = "save " + save.translate(translate_table)[:1] + " is possible ..."
        gif_name = queue + ".gif"
        command = 'node ../fumen-canvas/fumen-canvas.js gif ' + fumen + ' ' + gif_name + ' --start ' + str(i) + ' --end ' + str(i+10)
        os.system(command) # run the command
        my_package.media_files.append(gif_name) # add this image to the packages media files
        queue = '<a href="https://himitsuconfidential.github.io/downstack-practice/usermode.html/=' + queue + '" > PC# ' + str(int((i + 10) / 10)) + '</a>'
        my_note = genanki.Note(
            model=my_model,
            fields=[queue + " " + save, '<img src ="' + gif_name + '">'])
